...
minn = 30
plot_x_1 = []		# For plotting the result.
plot_y_1 = []
train_n = (24*20-hoursfortrain)*1
valid_n = (24*20-hoursfortrain)*4
for iternum in range(300000):
	K = ((np.dot(Weights,traindata[:,0:train_n])+np.ones(train_n)*bias)*-1+pm25data[0:train_n])*-2
	par_Weights = np.dot(traindata[:,0:train_n],K)
	...
	if(iternum%10000==0):
		result_K = ((np.dot(Weights,traindata[:,-valid_n:])+np.ones(valid_n)*bias)*-1+pm25data[-valid_n:])
		result_total = np.sqrt(np.sum(result_K**2) / np.shape(result_K)[0])
		plot_x_1.append(iternum)	
		plot_y_1.append(result_total)
...
testoutcome = np.dot(Weights,testdata)+np.zeros(240)*bias
